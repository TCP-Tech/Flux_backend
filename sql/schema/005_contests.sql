...
CREATE INDEX idx_contest_start_time ON contest(start_time);
CREATE INDEX idx_contest_end_time ON contest(end_time);

-- A trigger to automatically update 'updated_at' on every row modification for contest table
CREATE TRIGGER update_contest_updated_at BEFORE UPDATE ON contest FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE contest_registered_users (
    ...
    contest_id UUID NOT NULL REFERENCES contest(id), -- The contest they are registered for
...
DROP TRIGGER update_contest_updated_at ON contest;
